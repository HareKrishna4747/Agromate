...
st.set_page_config("AgroMate - Corn Disease Predictor", page_icon="🌽")
st.markdown("""
    <style>
...
st.title("🌽 AgroMate - AI Corn Disease Chatbot")
st.write("Upload an image of a corn leaf to detect disease and chat about treatment suggestions.")

# Load AI Model
@st.cache_resource
def load_model():
    return tf.keras.models.load_model("corn.h5")

model = load_model()

# Define Classes for Corn Disease
corn_classes = [
...
def query_chatgpt(label, user_input=None):
    if user_input:
        prompt = f"{user_input} (Context: Detected {label} in a corn plant)"
    else:
        prompt = f"I detected {label} in a corn plant. What are the treatments and precautions?"
    
    response = llm.invoke(prompt)
    return response.content  # Extract only text content

# Image Upload
uploaded_file = st.file_uploader("Upload a corn leaf image", type=["jpg", "png", "jpeg"])
...
if uploaded_file:
    ...
    label = corn_classes[predicted_class]
